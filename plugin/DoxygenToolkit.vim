...
function! s:ParseFunctionParameters( lineBuffer, doc )
    ...
    if( s:CheckFileType() == "cpp" )
        ...
        let l:functionBuffer        = substitute ( l:functionBuffer, '^[[:blank:]]*\|[[:blank:]]*$','','' )
        let l:functionReturnAndName = split ( l:functionBuffer, '[[:blank:]*]' )[-1]
        if( len( l:functionReturnAndName ) > 1 )
            let a:doc.returns = 'yes'
        endif
        " let a:doc.name = functionReturnAndName[-1]
        let a:doc.name = functionBuffer
    endif
